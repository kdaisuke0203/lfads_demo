...
class LFADS_Net(nn.Module):
    ...
    def __init__(self, inputs_dim, T, dt,
        ...
        self.fc_logrates = nn.Sequential(nn.Linear(self.factors_dim,350),nn.ReLU(),
                                          nn.Dropout(0.2),
                                  nn.Linear(350,350),nn.ReLU(),
                                         nn.Dropout(0.2),
                                  nn.Linear(350,self.inputs_dim))
        # -----------
        # Dropout layer
        # -----------
...
#         pdb.set_trace()
    ...
    def generate(self, x):
        ...
        for t in range(self.T):
            ...
            self.kl_loss = self.kl_loss + 0.0*KLCostGaussian(self.u_mean, self.u_logvar,
            ...
            if t>0:
                ...
                self.klf_loss = self.klf_loss + 0.0*KLCostGaussian(f_his_mean[t], f_his_logvar[t],
                                            self.f_prior_mean1, self.f_prior_logvar1)/x.shape[0]
                for i in range(b_size):
                    if x[i,t,37] >0:
                        self.smo_loss +=  0.0*x[i,t,37]*((f_his[t][i][0]-f_his[t-1][i][0])**2).sum()
                    else:
                        self.smo_loss +=  0.0*((f_his_mean[t]-f_his_mean[t-1])**2).sum()
                for i in range(b_size):
                    self.dir_loss += 0.0*(abs((f_his[t][i][0]-f_his[t-1][i][0])/(f_his[t-1][i][0]+1e-6))*(x[i,t,4]+x[i,t,10]+x[i,t,22]+x[i,t,31]) +
                                        abs((f_his[t][i][1]-f_his[t-1][i][1])/(f_his[t-1][i][1]+1e-6))*(x[i,t,1]+x[i,t,2]+x[i,t,5]+x[i,t,6]+x[i,t,13]+x[i,t,15]+x[i,t,18]+x[i,t,23])).sum()
    ...
    def fit(self, train_dataset, valid_dataset,
        ...
        for epoch in range(max_epochs):
            ...
            for i, x in enumerate(train_dl, 0):
                ...
                loss = self.recon_loss + kl_weight * self.kl_loss + l2_weight * l2_loss + kl_weight *self.klf_loss + self.smo_loss+ self.dir_loss
            ...
            print('recon: %4d, kl: %5d, dir: %4d, smo: %4d, klf: %4d, klw: %3f' %(self.recon_loss, kl_weight * self.kl_loss, self.dir_loss, self.smo_loss, kl_weight *self.klf_loss,kl_weight))
